...
All components (with the exception of <<Text>>) are represented as JSON objects
with a special "@type" key that identifies the type of component. For example,
an image component might be represented as the the following JSON object:
